          ...
          try {
            ...
            try {
              ...
              if (response.statusCode == 200) {
                ...
                lastErrorMessage = responseData['message'] ?? responseData['error'] ?? 'Failed to login. Status code: ${response.statusCode}';
                lastStatusCode = response.statusCode;
              }
            } catch (parseError) {
              lastErrorMessage = 'Server returned invalid format. Status code: ${response.statusCode}';
              lastStatusCode = response.statusCode;
            }
          } catch (e) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          final rootResponse = await client.get(rootUrl)
              .timeout(const Duration(seconds: 5));
